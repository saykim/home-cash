...
import { getNextOccurrence, getDaysUntilEvent, getEventTypeLabel } from '@/lib/eventUtils';
...
export function EventListItem({ event, onToggle, onDelete }: EventListItemProps) {
  ...
  return (
    <div className="flex items-center justify-between p-3 border rounded-lg group hover:bg-muted/30 transition-colors">
      <div className="flex items-center gap-3">
        <div
          className={cn(
            ...
            event.isActive ? 'bg-pink-100 dark:bg-pink-900/30' : 'bg-gray-100 dark:bg-gray-900/30'
          )}
        >
          <Calendar
            className={cn(
              'h-4 w-4',
              event.isActive ? 'text-pink-600 dark:text-pink-400' : 'text-gray-400'
            )}
          />
        </div>
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span>{event.month}월 {event.day}일</span>
            <span>·</span>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <Power className={cn('h-4 w-4', event.isActive ? 'text-green-600' : 'text-gray-400')} />
        </Button>
        <Button
          variant="ghost"
          size="icon"
