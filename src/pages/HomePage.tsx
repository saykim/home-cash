...
import { detectRecurringPatterns, isPatternAlreadyRegistered } from '@/lib/recurringDetection';
import { Wallet, TrendingUp, TrendingDown, CreditCard, List, BarChart3, Calendar, Bell, ArrowUpRight, ArrowDownRight, RefreshCw } from 'lucide-react';
...
export default function HomePage() {
  ...
  const upcomingPayments = performances
    .map((perf) => {
      const daysUntil = differenceInDays(parseISO(perf.nextBillingDate), new Date());
      return {
        ...perf,
        daysUntil
      };
  ...
  const suggestedRecurring = useMemo(() => {
    ...
    return patterns.filter((pattern) => !isPatternAlreadyRegistered(pattern, recurringTransactions));
      ...
      <div className="grid grid-cols-12 gap-4">
          ...
          <Card className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 text-white border-0 shadow-xl h-full">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-300 uppercase tracking-wider">총 자산</span>
        ...
        <div className="col-span-7 grid grid-cols-3 gap-4">
          ...
          <Card className="border border-green-200 dark:border-green-900/50 bg-green-50/50 dark:bg-green-950/20">
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-green-700 dark:text-green-400 uppercase tracking-wider">월 수입</span>
                <ArrowUpRight className="h-4 w-4 text-green-600 dark:text-green-400" />
              </div>
              <div className="text-2xl font-bold text-green-700 dark:text-green-400 tabular-nums">{formatCurrency(monthIncome)}</div>
            </CardContent>
          </Card>
          ...
          <Card className="border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/20">
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-red-700 dark:text-red-400 uppercase tracking-wider">월 지출</span>
                <ArrowDownRight className="h-4 w-4 text-red-600 dark:text-red-400" />
              </div>
              <div className="text-2xl font-bold text-red-700 dark:text-red-400 tabular-nums">{formatCurrency(monthExpense)}</div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-1">
                <span className={cn(
                  ...
                  netIncome >= 0 ? "text-blue-700 dark:text-blue-400" : "text-orange-700 dark:text-orange-400"
                )}>순수익</span>
        ...
        <div className="col-span-8 space-y-6">
          ...
          <Card className="border">
            <CardHeader className="pb-3 border-b">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base font-semibold">자산 현황</CardTitle>
          ...
          <Card className="border">
            ...
            <CardContent className="p-0">
              {recentTransactions.length === 0 ? (
                <div className="p-8 text-center text-sm text-muted-foreground">거래 내역이 없습니다</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        <th className="text-left py-4 px-5 text-sm font-medium text-muted-foreground">날짜</th>
                        <th className="text-left py-4 px-5 text-sm font-medium text-muted-foreground">내용</th>
                        <th className="text-left py-4 px-5 text-sm font-medium text-muted-foreground">구분</th>
                        <th className="text-right py-4 px-5 text-sm font-medium text-muted-foreground">금액</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {recentTransactions.map((tx) => {
                        ...
                        return (
                          <tr key={tx.id} className="group hover:bg-muted/30 hover:shadow-sm transition-all cursor-pointer relative">
                            <td className="relative py-4 px-5">
                              <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary opacity-0 group-hover:opacity-100 transition-opacity rounded-r" />
                            ...
                            <td className="py-4 px-5">
                              <span className="font-medium text-base">{tx.memo || category?.name || '거래'}</span>
                            </td>
                            <td className="py-4 px-5">
                              <span className={cn(
                                ...
                                tx.type === 'TRANSFER' && "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400"
                              )}>
                  ...
                  {upcomingEvents.map((event) => (
                    <div
                      ...
                      className={cn(
                        ...
                        event.daysUntil === 0 && 'bg-pink-100 dark:bg-pink-900/30 border-pink-300 dark:border-pink-800',
                        event.daysUntil > 0 && event.daysUntil <= 3 && 'bg-orange-100 dark:bg-orange-900/30 border-orange-300 dark:border-orange-800',
                        event.daysUntil > 3 && 'bg-blue-100 dark:bg-blue-900/30 border-blue-300 dark:border-blue-800'
                      )}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            {getEventTypeLabel(event.type)} · {format(event.nextDate, 'M/d (E)', { locale: ko })}
                          </p>
                        </div>
                        <div className={cn(
                          'px-2 py-1 rounded text-xs font-bold',
                          event.daysUntil === 0 && 'bg-pink-600 text-white',
                          event.daysUntil > 0 && event.daysUntil <= 3 && 'bg-orange-600 text-white',
                      ...
                      {event.amount && (
                        <div className="text-right">
                          ...
                          <p className="font-bold text-base tabular-nums">{formatCurrency(event.amount)}</p>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="space-y-2">
                  {upcomingPayments.map((payment) => (
                    <div
                      ...
                      className={cn(
                        'p-3 rounded-lg border transition-all',
                        payment.daysUntil === 0 && 'bg-red-100 dark:bg-red-900/30 border-red-300 dark:border-red-800',
                        payment.daysUntil > 0 && payment.daysUntil <= 3 && 'bg-orange-100 dark:bg-orange-900/30 border-orange-300 dark:border-orange-800',
                        payment.daysUntil > 3 && 'bg-blue-100 dark:bg-blue-900/30 border-blue-300 dark:border-blue-800'
                      )}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <p className="font-semibold text-sm leading-tight">{payment.cardName}</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            {format(parseISO(payment.nextBillingDate), 'M/d (E)', { locale: ko })}
                          </p>
                        </div>
                        <div className={cn(
                          'px-2 py-1 rounded text-xs font-bold',
                          payment.daysUntil === 0 && 'bg-red-600 text-white',
                          payment.daysUntil > 0 && payment.daysUntil <= 3 && 'bg-orange-600 text-white',
                          payment.daysUntil > 3 && 'bg-blue-600 text-white'
                        )}>
                          {payment.daysUntil === 0 ? '오늘' : `D-${payment.daysUntil}`}
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-base tabular-nums">{formatCurrency(payment.billingAmount)}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {suggestedRecurring.length > 0 && (
            <Card className="border border-purple-200 dark:border-purple-900/50 bg-purple-50/30 dark:bg-purple-950/10">
              <CardHeader className="pb-3 border-b border-purple-200 dark:border-purple-900/50">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  최근 {suggestedRecurring.length}개의 반복 패턴이 감지되었습니다
                </p>
              </CardHeader>
              <CardContent className="p-3">
                <div className="space-y-2">
                  {suggestedRecurring.slice(0, 3).map((pattern) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <p className="font-semibold text-sm leading-tight">{pattern.categoryName}</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            최근 {pattern.occurrences}개월 연속 발생 · 신뢰도 {pattern.confidence}%
                          </p>
                        </div>
